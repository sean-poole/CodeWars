...
/*
P - One argument as an integer n >= 10 && n < 10000. 
R - String value of key n in the `fruits` object. If n is not listed, subtract the sum of the digits in n from n and repeat as necessary.
E - SubtractSum(10)     // "apple"
P - 
function SubtractSum(n) {
    // Subtract the sum of the digits in n from the given n.
    // Check if the new value is stored in the fruits object.
    // If it is, return the associated string value.
    // Else, continue subtracting the sum of the digits from n until a match is made.
}
*/
...
function SubtractSum(n) {
    let number = n - n.toString().split("").reduce((acc, c) => acc += +(c), 0);
    return number > 100 ? SubtractSum(number) : fruits[number];
}

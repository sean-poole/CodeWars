...
function balancedNum(number) {
    ...
    let mid = Math.ceil(number.length / 2) - 1;
    let head = [];
    let tail = [];
    if (number.length % 2) {
        head = number.slice(0, mid);
        tail = number.slice(mid + 1);
    } else {
        head = number.slice(0, mid);
        tail = number.slice(mid + 2);
    }

    return head.reduce((acc, c) => acc += c, 0) === tail.reduce((acc, c) => acc += c, 0) ? "Balanced" : "Not Balanced";
}

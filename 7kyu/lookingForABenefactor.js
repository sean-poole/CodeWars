...
function newAvg(arr, newavg) {
    let donations = arr.reduce((acc, c) => acc += c, 0);
    if (donations / arr.length > newavg) throw "Expected New Average is too low";

    return Math.ceil(newavg * (arr.length + 1) - donations);
}
